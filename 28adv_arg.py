# 함수 고급기능 - 키워드 인수
# 함수 호출시 전달되는 값이
# 어떤 매개변수에 지정되는 지 결정 : 순서대로 할당
# 인수의 순서를 바꿔 함수를 호출할 수 있음 호출할 수 있음

def addv1(a,b):
    print(f'{a} + {b} = {a+b} ')

addv1(3,5)
addv1(a=3, b=5)     # 인수가 전달될때 매개변수명 지정
...
addv2()         # a,b 매개변수에 미리 정해둔 4,6이 전달됨
addv2(b=10)     # a 매개변수에 미리 정해둔 4가 전달됨

...
def modifySungJuk():
    ...
    conn, cur = db.openConn()
    sql = "update sungjuk set kor=%(kr)s, eng=%(en)s, mat=%(mt)s, tot=%(tt)s, avg=%(av)s, grd=%(gd)s,  regdate = current_timestamp " \
          "where name =%(nm)s"
